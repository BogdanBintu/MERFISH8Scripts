...
from AlignmentTools_py3 import *
...
def med_correct(im3d,ksize = 32):
    im3d_=np.array(im3d,dtype=float)
    if len(im3d.shape)==2:
        im3d_ = im3d[np.newaxis,...]
    sz,sxo,syo = im3d_.shape
...
def get_ims_meds(folder,num_cols=None,fovs=None,median_fl=None,overwrite=False,tag='mid',verbose=False):
    ...
    if compute:
        ...
        imst_cols=[]
        if fovs is None:
            fovs = [os.path.basename(temp_) for temp_ in glob.glob(folder+os.sep+'*.dax')]
        for fov in fovs:
            if verbose:
                print(fov)
...
def update_candid_spot(im,chr_pts_,txyz_,tag,th_std=2,num_pts=5,
    ...
    for ichr,chr_pt in enumerate(chr_pts_):
        ...
        if np.prod(im_sm.shape)>0:
            ...
            im_medz = np.array([np.median(im___) for im___ in im_sm])[:,np.newaxis,np.newaxis]
            im_sm = im_sm/im_medz
...
def get_cand_pts(im_sm,cr1,tzxy,dist_th = 5):
    ...
    cr1,cr2 = [np.array(pt__) for pt__ in zip(*pairs)]
...
def get_STD_beaddrift_v2(ims_beads,coord_sel=None,sz_ex=50,desired_nbeads = 20, 
    ...
    if repeat:
        #get txyz
        if ref is None: ref=int(len(ims_beads)/2)
        ...
        for iim in bad_inds+list(range(len(txyzs_both),len(ims_beads))):
            ...
            if (displ>desired_displ) or (c11<desired_nbeads) or (c21<desired_nbeads):
                ...
                def l1d(x): return np.sum(np.abs(x))
                measures = [l1d(txyz3-txyz1),l1d(txyz3-txyz2),l1d(txyz1-txyz2)]
...
def get_best_drift(drifts_):
    imax = np.max([len(drft) for drft in drifts_])
    return [get_drift_median(drifts_,i) for i in range(imax)]
