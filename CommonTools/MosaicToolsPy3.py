...
import cv2
def compose_mosaic(ims,ims_c,xs_um,ys_um,um_per_pix=0.153,rot = -2,plt_val=False,tag=''):
    im_ = ims[0]
    dtype_ = im_.dtype
    sx,sy = im_.shape

    xs_pix = np.array(xs_um)/um_per_pix
    xs_pix = np.array(xs_pix-np.min(xs_pix),dtype=int)
    ys_pix = np.array(ys_um)/um_per_pix
    ys_pix = np.array(ys_pix-np.min(ys_pix),dtype=int)
    sx_big = np.max(xs_pix)+sx+1
    sy_big = np.max(ys_pix)+sy+1
    im_big = np.zeros([sx_big,sy_big],dtype = 'float32')

    h,w = im_.shape
    center = (w / 2., h / 2.)
    M = cv2.getRotationMatrix2D(center,rot, 1.)
    for im_,x_,y_ in zip(ims,xs_pix,ys_pix):
        if ims_c is not None:
            im__=np.array(im_,dtype = 'float32')/np.array(ims_c,dtype = 'float32')
        else:
            im__=np.array(im_,dtype = 'float32')
        if tag=='dax':
            im__ = im__[::1,::-1]
        im__ = cv2.warpAffine(im__, M, (h, w))
        
        im_big[x_:x_+sx,y_:y_+sy]=np.max([im__,im_big[x_:x_+sx,y_:y_+sy]],0)
